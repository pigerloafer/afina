...
ServerImpl::ServerImpl(std::shared_ptr<Afina::Storage> ps, std::shared_ptr<Logging::Service> pl)
    : Server(ps, pl), running(true), num_of_workers(0) {}
    ...
    while (running.load()) {
        ...
        if (num_of_workers.load() <= MAX_NUM_OF_WORKERS) {
            ...
            std::thread client([=]() {
                    ...
                    while ((readed_bytes = read(client_socket, client_buffer, sizeof(client_buffer))) > 0) {
                        ...
                        while (readed_bytes > 0) {
                            ...
                            if (!command_to_execute) {
                                ...
                                // Parsed might fails to consume any bytes from input stream. In real life that could
                                // happens, for example, because we are working with UTF-16 chars and only 1 byte left
                                // in stream
                                if (parsed == 0) {
                                    break;
                                } else {
                ...
                // Prepare for the next command: just in case if connection was closed in the middle of executing
                // something
                command_to_execute.reset();
                argument_for_command.resize(0);
                parser.Reset();
